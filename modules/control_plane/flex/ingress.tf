...
resource "aws_cloudwatch_log_group" "public_ingress_api" {
  name              = "/aws/apigateway/${var.stack_name}-public-ingress"
  retention_in_days = 14

  tags = merge(
    local.common_tags,
    {
      Name = "${var.stack_name}-public-ingress-api"
    }
  )
}
...
resource "aws_api_gateway_deployment" "public_ingress" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.setup_proxy,
    aws_api_gateway_integration.readyz,
  ]
}
...
resource "aws_api_gateway_stage" "public_ingress" {
  ...
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.public_ingress_api.arn
    format = jsonencode({
